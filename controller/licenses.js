...
exports.readLicenses = function(req, res) {
    ...
    try{
        connection.query('SELECT * FROM Licenses WHERE user_id = ?', [user_id]
...
exports.updateLicenses = function(req, res) {
    ...
    try{
        connection.query('UPDATE Licenses SET name = ?, issue_date = ?, exp_date = ?, organization = ?, description = ?, credential = ? WHERE id = ?',
        [ name, issue_date, exp_date, organization, description, credential, id ],
        function (error, rows, fields){
            if(error){
                response.internalError(error.code,res);
            } else{
